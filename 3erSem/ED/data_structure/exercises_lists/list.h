 ...
 *      Author: pperezm
 *
 *  Modified by: Hugo David Franco Avila
 *  Date: 06 - 09 - 18
 *
 *
 */
...
public:
	...
	void set(int, T) throw (IndexOutOfBounds);//cambio de bool
	int  indexOf(T) const;
...
template <class T>
void List<T>::set(int index, T val) throw (IndexOutOfBounds) {//cambio a void
	Link<T> *p;
	int i = 0;
	if(index < 0 || index >= size){
		throw IndexOutOfBounds();
